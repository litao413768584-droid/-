...
export const CalculatorView: React.FC<CalculatorViewProps> = ({
  ...
  const [globalInput, setGlobalInput] = useState<ShipGlobalInput>(() => {
    ...
    return {
      ...
      syncWithFirstTank: false, // 默认不强制同步，每个舱标密/温度均可自由输入
      correctWaterSounding: true, // 默认水高双向倾斜修正
  ...
  const handleTankValueChange = (tankId: string, field: keyof TankInput, value: any) => {
    setTankInputs(prev => {
      const firstTankId = prev[0]?.tankId;
      const isFirstTank = tankId === firstTankId;
      const shouldSync = globalInput.syncWithFirstTank && isFirstTank && (field === 'density20C' || field === 'temperature' || field === 'vcf');
      ...
      return prev.map(t => {
        ...
        if (t.tankId === tankId) {
          if (field === 'waterVolume' && value !== '' && value !== undefined) {
            return { ...t, waterVolume: value, waterSounding: '' as any };
          }
          return { ...t, [field]: value };
        }
        return t;
  ...
  return (
    <div className="space-y-6 pb-12">
      ...
      <TankVisualizer
        vesselName={customVesselName || summary.vesselName}
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl p-5 text-white shadow-xl overflow-hidden">
        ...
        <div className="bg-slate-950 border border-slate-800 rounded-lg p-4 mb-5">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-3.5 mt-4 pt-3.5 border-t border-slate-800/80">
            ...
            {/* 水高是否参与纵横倾修正 (新增) */}
            <div className="flex items-center justify-between bg-slate-900 p-2 rounded border border-slate-800">
              <span className="text-xs font-semibold text-slate-300">水高倾斜修正:</span>
              <label className="flex items-center gap-1.5 cursor-pointer bg-slate-950 px-2 py-1 rounded border border-slate-700 hover:border-slate-500 transition-colors" title="开启后水高将自动经纵横倾双向修正">
                <input
                  type="checkbox"
                  checked={globalInput.correctWaterSounding ?? true}
                  onChange={e => setGlobalInput(prev => ({ ...prev, correctWaterSounding: e.target.checked }))}
                  className="rounded border-slate-700 text-cyan-500 focus:ring-cyan-500 bg-slate-900 w-4 h-4 cursor-pointer"
                />
                <span className={`text-xs font-bold ${globalInput.correctWaterSounding ?? true ? 'text-cyan-300' : 'text-slate-400'}`}>
                  {globalInput.correctWaterSounding ?? true ? '是 (修正)' : '否 (原始)'}
                </span>
              </label>
            </div>
        ...
        <div className="overflow-x-auto rounded-lg">
          <table className="w-full text-left text-xs border-collapse font-sans table-auto">
            <thead>
              <tr className="bg-slate-950 text-slate-300 border-b-2 border-slate-800 font-bold text-[9px] sm:text-[10px] uppercase tracking-wider align-bottom">
                <th className="py-1.5 px-0.5 leading-tight whitespace-nowrap min-w-[40px] w-[40px]">
                  舱号<br /><span className="text-[7px] font-normal text-slate-400">(Tank)</span>
                </th>
                <th className="py-1.5 px-0.5 text-center leading-tight whitespace-nowrap min-w-[28px] w-[28px]">类型</th>
                <th className="py-1.5 px-0.5 text-right leading-tight whitespace-nowrap min-w-[38px] w-[38px]">测量值<br /><span className="text-[7px] font-normal text-slate-400">(mm)</span></th>
                <th className="py-1.5 px-0.5 text-right text-blue-300 leading-tight whitespace-nowrap min-w-[36px] w-[36px]">纵/横倾<br /><span className="text-[7px] text-blue-300/80">修正(mm)</span></th>
                <th className="py-1.5 px-0.5 text-right text-emerald-300 leading-tight whitespace-nowrap min-w-[48px] w-[48px]">
                  20°C标密<br />
                  <span className="text-[7px] font-normal text-slate-400">(t/m³)</span>
                </th>
                <th className="py-1.5 px-0.5 text-right text-amber-300 leading-tight whitespace-nowrap min-w-[34px] w-[34px]">温度<br /><span className="text-[7px] font-normal text-slate-400">(°C)</span></th>
                <th className="py-1.5 px-0.5 text-right leading-tight whitespace-nowrap min-w-[58px] w-[58px]">观测体积<br /><span className="text-[7px] font-normal text-slate-400">OBS.VOL</span></th>
                <th className="py-1.5 px-0.5 text-right text-cyan-300 leading-tight whitespace-nowrap min-w-[36px] w-[36px]">水高<br /><span className="text-[7px] font-normal text-cyan-400">(mm)</span></th>
                <th className="py-1.5 px-0.5 text-right text-cyan-200 leading-tight whitespace-nowrap min-w-[38px] w-[38px]">修正水高<br /><span className="text-[7px] font-normal text-cyan-300">(mm)</span></th>
                <th className="py-1.5 px-0.5 text-right text-cyan-100 leading-tight whitespace-nowrap min-w-[58px] w-[58px]">明/扣水<br /><span className="text-[7px] font-normal text-slate-400">(m³)</span></th>
                <th className="py-1.5 px-0.5 text-right text-blue-200 font-bold leading-tight whitespace-nowrap min-w-[58px] w-[58px]">实际体积<br /><span className="text-[7px] font-normal text-blue-300 font-mono">G.O.V</span></th>
                <th className="py-1.5 px-0.5 text-right text-emerald-400 leading-tight whitespace-nowrap min-w-[44px] w-[44px]">钢材膨胀<br /><span className="text-[7px] font-normal text-slate-400">TK EXP.</span></th>
                <th className="py-1.5 px-0.5 text-right text-amber-200 leading-tight whitespace-nowrap min-w-[44px] w-[44px]">重量修正<br /><span className="text-[7px] font-normal text-slate-400">WCF</span></th>
                <th className="py-1.5 px-0.5 text-right text-purple-300 leading-tight whitespace-nowrap min-w-[46px] w-[46px]">体积修正<br /><span className="text-[7px] font-normal text-slate-400">VCF20</span></th>
                <th className="py-1.5 px-0.5 text-right text-purple-200 font-bold leading-tight whitespace-nowrap min-w-[58px] w-[58px]">总标体积<br /><span className="text-[7px] font-normal text-purple-300 font-mono">G.S.V</span></th>
                <th className="py-1.5 px-0.5 text-right text-emerald-300 font-black text-[9px] sm:text-[10px] bg-emerald-950/20 leading-tight whitespace-nowrap min-w-[60px] w-[60px]">
                  净油重(t)<br />
                  <span className={`text-[7px] font-normal ${globalInput.useSteelExpansion ? 'text-emerald-400' : 'text-amber-400 font-bold'}`}>
                    {globalInput.useSteelExpansion ? '(钢膨后)' : '(无钢膨)'}
                  </span>
                </th>
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-800/60 font-mono text-slate-100 text-xs sm:text-[12px]">
              {summary.tankResults.map((tank, idx) => {
                const isFirst = idx === 0;
                const isDisabledSync = !isFirst && globalInput.syncWithFirstTank;
                const rawInput = tankInputs.find(i => i.tankId === tank.tankId);
                ...
                return (
                  ...
                  >
                    {/* 舱号 */}
                    <td className="py-1.5 px-0.5 font-sans font-bold text-white whitespace-nowrap text-xs sm:text-[12px] leading-tight">
                      <div className="flex items-center gap-0.5">
                        <span>{tank.tankName}</span>
                        {isFirst && globalInput.syncWithFirstTank && (
                          <span className="text-[7px] bg-purple-500/20 text-purple-300 px-0.5 py-0 rounded border border-purple-500/30 font-mono shrink-0">
                            基准
                          </span>
                        )}
                      </div>
                    ...
                    {/* 类型 */}
                    <td className="py-1.5 px-0.5 text-center whitespace-nowrap">
                      <button
                        type="button"
                        onClick={(e) => {
                          e.stopPropagation();
                        ...
                        className={`px-0.5 py-0.5 text-[9px] font-semibold rounded border transition-colors shadow-sm leading-none ${
                          tank.type === 'sounding'
                    ...
                    {/* 测量值 (mm) */}
                    <td className="py-1.5 px-0.5 text-right w-[38px]">
                      <input
                        type="number"
                        step="1"
                        min="0"
                        ...
                        onChange={(e) => {
                          handleTankValueChange(tank.tankId, 'value', e.target.value);
                        }}
                        onClick={(e) => e.stopPropagation()}
                        placeholder="0"
                        className="w-full min-w-0 bg-slate-950 border border-slate-700 hover:border-blue-500 focus:border-blue-400 rounded px-0.5 py-0.5 text-right text-xs sm:text-[12px] text-white font-mono font-bold focus:outline-none focus:ring-1 focus:ring-blue-400 shadow-inner [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none"
                      />
                    </td>

                    {/* 纵/横倾修正 (mm) */}
                    <td
                      className="py-1.5 px-0.5 text-right font-bold text-blue-300 text-xs sm:text-[12px] w-[36px]"
                      title={`修正后测量值: ${(tank.correctedSounding * 1000).toFixed(1)} mm`}
                    >
                      {(tank.totalCorrection >= 0 ? '+' : '') + tank.totalCorrection.toFixed(1)}
                    </td>

                    {/* 20°C标密 (t/m³) */}
                    <td className="py-1.5 px-0.5 text-right w-[48px]">
                      <input
                        type="number"
                        step="0.0001"
                        min="0.5"
                        ...
                        value={isDisabledSync ? tank.density20C : (rawInput?.density20C ?? '')}
                        onChange={(e) => {
                          handleTankValueChange(tank.tankId, 'density20C', e.target.value);
                        }}
                        onClick={(e) => e.stopPropagation()}
                        placeholder="0.8500"
                        className={`w-full min-w-0 bg-slate-950 border rounded px-0.5 py-0.5 text-right text-xs sm:text-[12px] font-mono font-bold focus:outline-none shadow-inner [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none ${
                          isDisabledSync
                    ...
                    {/* 温度 (°C) */}
                    <td className="py-1.5 px-0.5 text-right w-[34px]">
                      <input
                        type="number"
                        step="0.1"
                        disabled={isDisabledSync}
                        value={isDisabledSync ? tank.temperature : (rawInput?.temperature ?? '')}
                        onChange={(e) => {
                          handleTankValueChange(tank.tankId, 'temperature', e.target.value);
                        }}
                        onClick={(e) => e.stopPropagation()}
                        placeholder="20.0"
                        className={`w-full min-w-0 bg-slate-950 border rounded px-0.5 py-0.5 text-right text-xs sm:text-[12px] font-mono font-bold focus:outline-none shadow-inner [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none ${
                    ...
                    {/* 观测体积 OBS. VOL. (m³) */}
                    <td className="py-1.5 px-0.5 text-right text-slate-100 font-bold text-xs sm:text-[12px] w-[58px]">
                      {tank.obsVolume.toFixed(3)}
                    </td>

                    {/* 水高 (mm) 输入 */}
                    <td className="py-1.5 px-0.5 text-right w-[36px]">
                      <input
                        type="number"
                        step="0.1"
                        min="0"
                        value={rawInput?.waterSounding ?? ''}
                        onChange={(e) => {
                          handleTankValueChange(tank.tankId, 'waterSounding', e.target.value);
                        }}
                        onClick={(e) => e.stopPropagation()}
                        placeholder="0"
                        className="w-full min-w-0 bg-slate-950 border border-cyan-700/60 hover:border-cyan-500 focus:border-cyan-400 rounded px-0.5 py-0.5 text-right text-xs sm:text-[12px] text-cyan-200 font-mono font-bold focus:outline-none shadow-inner [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none"
                        title="录入测得的水高(mm)"
                      />
                    </td>
                    ...
                    {/* 修正水高 (mm) 新增列 */}
                    <td
                      className="py-1.5 px-0.5 text-right font-bold text-cyan-300 text-xs sm:text-[12px] w-[38px]"
                      title={`水高倾斜修正量: ${tank.waterCorrectionMm >= 0 ? '+' : ''}${tank.waterCorrectionMm.toFixed(1)} mm`}
                    >
                      {tank.correctedWaterSoundingMm.toFixed(1)}
                    </td>

                    {/* 明水/扣水 (m³) */}
                    <td className="py-1.5 px-0.5 text-right w-[58px]">
                      <input
                        type="number"
                        step="0.001"
                        min="0"
                        value={
                          parseFloat((rawInput?.waterSounding || 0) as any) > 0
                            ? tank.waterVolume.toFixed(3)
                            : (rawInput?.waterVolume ?? '')
                        }
                        onChange={(e) => {
                          handleTankValueChange(tank.tankId, 'waterVolume', e.target.value);
                        }}
                        onClick={(e) => e.stopPropagation()}
                        placeholder="0.000"
                        className={`w-full min-w-0 bg-slate-950 border rounded px-0.5 py-0.5 text-right text-xs sm:text-[12px] font-mono font-bold focus:outline-none shadow-inner [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none ${
                        ...
                        title={parseFloat((rawInput?.waterSounding || 0) as any) > 0 ? '自动计算明水体积，可覆盖' : '直接输入明水体积'}
                      />
                    </td>

                    {/* 实际体积 G.O.V (m³) */}
                    <td className="py-1.5 px-0.5 text-right font-bold text-blue-200 bg-blue-500/5 text-xs sm:text-[12px] w-[58px]">
                      {tank.govVolume.toFixed(3)}
                    </td>

                    {/* 钢材膨胀系数 TK EXP. */}
                    <td className="py-1.5 px-0.5 text-right font-mono font-semibold text-emerald-400 text-[10px] sm:text-[11px] w-[44px]">
                      {summary.useSteelExpansion ? tank.tempFactor.toFixed(5) : 'NIL'}
                    </td>

                    {/* 重量修正系数 WCF */}
                    <td className="py-1.5 px-0.5 text-right font-mono font-bold text-amber-300 text-[10px] sm:text-[11px] w-[44px]">
                      {tank.wcfFactor.toFixed(4)}
                    </td>

                    {/* 体积修正系数 VCF (保留4位小数) */}
                    <td className="py-1.5 px-0.5 text-right w-[46px]">
                      <input
                        type="number"
                        step="0.0001"
                        min="0.5"
                        max="1.5"
                        disabled={isDisabledSync}
                        value={isDisabledSync ? tank.vcfFactor : (rawInput?.vcf ?? '')}
                        onChange={(e) => {
                          handleTankValueChange(tank.tankId, 'vcf', e.target.value);
                        }}
                        onClick={(e) => e.stopPropagation()}
                        placeholder={tank.vcfFactor ? tank.vcfFactor.toFixed(4) : "1.0000"}
                        className={`w-full min-w-0 bg-slate-950 border rounded px-0.5 py-0.5 text-right text-xs sm:text-[12px] font-mono font-bold focus:outline-none shadow-inner [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none ${
                          isDisabledSync
                            ? 'border-slate-800 text-slate-500 opacity-70 cursor-not-allowed'
                    ...
                    {/* 总标准体积 G.S.V (m³) */}
                    <td className="py-1.5 px-0.5 text-right font-bold text-purple-200 bg-purple-500/5 text-xs sm:text-[12px] w-[58px]">
                      {tank.gsvVolume.toFixed(3)}
                    </td>

                    {/* 净油重量 (t) */}
                    <td className="py-1.5 px-0.5 text-right font-extrabold text-emerald-300 text-xs sm:text-[12px] bg-emerald-500/10 w-[60px]">
            ...
            <tfoot>
              {/* 各舱小计 Row */}
              <tr className="bg-slate-950 font-bold border-t-2 border-slate-700 text-slate-200 text-xs sm:text-[12px]">
                <td colSpan={6} className="py-2 px-0.5 text-right font-sans text-slate-300">
                  各舱小计:
                </td>
                <td className="py-2 px-0.5 text-right text-slate-100 font-mono font-bold text-xs sm:text-[12px]">
                  {summary.totalObsVolume.toFixed(3)}
                </td>
                <td className="py-2 px-0.5 text-right text-slate-500 font-mono">
                  -
                </td>
                <td className="py-2 px-0.5 text-right text-slate-500 font-mono">
                  -
                </td>
                <td className="py-2 px-0.5 text-right text-slate-400 font-mono text-xs sm:text-[12px]">
                  {(summary.totalObsVolume - summary.totalGovVolume).toFixed(3)}
                </td>
                <td className="py-2 px-0.5 text-right text-blue-200 font-mono text-xs sm:text-[12px] font-bold bg-blue-500/10">
                  {summary.totalGovVolume.toFixed(3)}
                </td>
                <td colSpan={3} className="py-2 px-0.5 text-right text-slate-400 font-sans font-normal text-xs sm:text-[12px]">
                  小计:
                </td>
                <td className="py-2 px-0.5 text-right text-purple-200 font-mono text-xs sm:text-[12px] font-bold bg-purple-500/10">
                  {summary.totalGsvVolume.toFixed(3)}
                </td>
                <td className="py-2 px-0.5 text-right text-emerald-300 font-mono text-xs sm:text-[12px] font-black bg-emerald-500/15">
                  {summary.tanksTotalWeight.toFixed(3)}
                </td>
              </tr>

              {/* 管线 (Pipeline Volume Input) */}
              <tr className="bg-slate-950/60 font-semibold border-t border-slate-800 text-purple-200 text-xs sm:text-[12px]">
                <td className="py-1.5 px-0.5 font-sans font-bold text-purple-200 whitespace-nowrap text-xs sm:text-[12px] leading-tight">
                  <div className="flex flex-col gap-1">
                    <span>管线 (Pipeline)</span>
                    <button
                      type="button"
                      onClick={() => setShowPipelineModal(true)}
                      className="text-[9px] bg-blue-500/20 hover:bg-blue-500/30 text-blue-300 border border-blue-500/40 rounded px-1 py-0.5 inline-flex items-center gap-0.5 font-sans whitespace-nowrap transition-colors"
                      title="查看检定证书《管线容量》明细表并一键自动填入"
                    >
                ...
                <td className="py-1.5 px-0.5 text-center text-slate-500">-</td>
                <td className="py-1.5 px-0.5 text-right text-slate-500">-</td>
                <td className="py-1.5 px-0.5 text-right text-slate-500">-</td>
                {/* 标密 */}
                <td className="py-1.5 px-0.5 text-right w-[48px]">
                  <input
                    ...
                    onChange={(e) => {
                      setGlobalInput(prev => ({ ...prev, pipelineDensity: e.target.value }));
                    }}
                    placeholder={summary.pipelineDensity.toFixed(4)}
                    className="w-full min-w-0 bg-slate-950 border border-emerald-700 hover:border-emerald-500 text-emerald-300 focus:border-emerald-400 rounded px-0.5 py-0.5 text-right text-xs sm:text-[12px] font-mono font-bold focus:outline-none shadow-inner [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none"
                ...
                <td className="py-1.5 px-0.5 text-right w-[34px]">
                  <input
                    ...
                    onChange={(e) => {
                      setGlobalInput(prev => ({ ...prev, pipelineTemp: e.target.value }));
                    }}
                    placeholder={summary.pipelineTemp.toFixed(1)}
                    className="w-full min-w-0 bg-slate-950 border border-amber-700 hover:border-amber-500 text-amber-300 focus:border-amber-400 rounded px-0.5 py-0.5 text-right text-xs sm:text-[12px] font-mono font-bold focus:outline-none shadow-inner [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none"
                    title="管线温度 (°C)，留空跟随首舱"
                  />
                </td>
                {/* 观测体积 */}
                <td className="py-1.5 px-0.5 text-right font-mono font-bold text-slate-300 text-xs sm:text-[12px] w-[58px]">
                ...
                <td className="py-1.5 px-0.5 text-right w-[58px]">
                  <input
                    ...
                    onChange={e => {
                      setGlobalInput(prev => ({ ...prev, pipelineVolume: e.target.value }));
                    }}
                    placeholder="0.000"
                    className="w-full min-w-0 bg-slate-900 border border-blue-500/50 rounded px-0.5 py-0.5 text-right text-blue-100 font-mono font-bold text-xs sm:text-[12px] focus:outline-none focus:border-blue-300 shadow-inner [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none"
                    title="管线容积/实际体积 (m³)"
                  />
                </td>
                {/* 钢材膨胀系数 */}
                <td className="py-1.5 px-0.5 text-right font-mono text-emerald-400 text-[10px] sm:text-[11px] w-[44px]">
                ...
                <td className="py-1.5 px-0.5 text-right w-[46px]">
                  <input
                    ...
                    onChange={(e) => {
                      setGlobalInput(prev => ({ ...prev, pipelineVcf: e.target.value }));
                    }}
                    placeholder={summary.pipelineVcf.toFixed(4)}
                    className="w-full min-w-0 bg-slate-950 border border-purple-700 hover:border-purple-500 text-purple-300 focus:border-purple-400 rounded px-0.5 py-0.5 text-right text-xs sm:text-[12px] font-mono font-bold focus:outline-none shadow-inner [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none"
                    title="管线VCF，留空跟随首舱"
                  />
                </td>
                {/* 总标准体积 G.S.V */}
                <td className="py-1.5 px-0.5 text-right font-mono font-bold text-purple-300 text-xs sm:text-[12px] w-[58px]">
                  {summary.pipelineGsv.toFixed(3)}
                </td>
                {/* 净油重量 */}
                <td className="py-1.5 px-0.5 text-right font-mono font-bold text-emerald-300 text-xs sm:text-[12px] bg-emerald-500/10 w-[60px]">
                  {summary.pipelineWeight.toFixed(3)}
                </td>
              </tr>

              {/* 底油 (ROB Volume Input) */}
              <tr className="bg-slate-950/60 font-semibold border-t border-slate-800 text-amber-200 text-xs sm:text-[12px]">
                <td className="py-1.5 px-0.5 font-sans font-bold text-amber-200 whitespace-nowrap text-xs sm:text-[12px] leading-tight">
                  底油 (ROB)
                </td>
                <td className="py-1.5 px-0.5 text-center text-slate-500">-</td>
                <td className="py-1.5 px-0.5 text-right text-slate-500">-</td>
                ...
                <td className="py-1.5 px-0.5 text-right w-[48px]">
                  <input
                    ...
                    onChange={(e) => {
                      setGlobalInput(prev => ({ ...prev, bottomRobDensity: e.target.value }));
                    }}
                ...
                <td className="py-1.5 px-0.5 text-right w-[34px]">
                  <input
                    ...
                    onChange={(e) => {
                      setGlobalInput(prev => ({ ...prev, bottomRobTemp: e.target.value }));
                    }}
                ...
                <td className="py-1.5 px-0.5 text-right w-[58px]">
                  <input
                    ...
                    step="0.001"
                    min="0"
                    value={globalInput.bottomRobVolume ?? ''}
                    onChange={e => {
                      setGlobalInput(prev => ({ ...prev, bottomRobVolume: e.target.value }));
                    }}
                    placeholder="0.000"
                    className="w-full min-w-0 bg-slate-900 border border-amber-500/50 rounded px-0.5 py-0.5 text-right text-amber-100 font-mono font-bold text-xs sm:text-[12px] focus:outline-none focus:border-amber-300 shadow-inner [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none"
                    title="底油容积/实际体积 (m³)"
                  />
                </td>
                {/* 钢材膨胀系数 */}
                ...
                <td className="py-1.5 px-0.5 text-right w-[46px]">
                  <input
                    ...
                    onChange={(e) => {
                      setGlobalInput(prev => ({ ...prev, bottomRobVcf: e.target.value }));
                    }}
              ...
              <tr className="bg-slate-950 font-black border-t-2 border-slate-700 text-white text-xs sm:text-sm">
                <td colSpan={6} className="py-2.5 px-0.5 text-right font-sans text-xs sm:text-sm">
                  <span className="text-amber-300 font-black">总量 (Grand Total):</span>
                </td>
                <td className="py-2.5 px-0.5 text-right text-blue-300 text-xs sm:text-[13px] font-mono font-bold bg-blue-500/10">
                  {summary.totalObsVolume.toFixed(3)}
                </td>
                <td className="py-2.5 px-0.5 text-right text-slate-500 font-mono text-xs sm:text-[13px]">
                  -
                </td>
                <td className="py-2.5 px-0.5 text-right text-slate-500 font-mono text-xs sm:text-[13px]">
                  -
                </td>
                <td className="py-2.5 px-0.5 text-right text-slate-400 font-mono text-xs sm:text-[13px]">
                  {(summary.totalObsVolume - summary.totalGovVolume).toFixed(3)}
                </td>
                <td className="py-2.5 px-0.5 text-right text-blue-200 text-xs sm:text-[13px] font-mono font-black bg-blue-500/20">
                  {summary.totalVolume.toFixed(3)}
                </td>
                <td colSpan={3} className="py-2.5 px-0.5 text-right text-amber-300 text-[10px] sm:text-xs font-sans">
                  全舱总体积/总净重:
                </td>
                <td className="py-2.5 px-0.5 text-right text-purple-200 text-xs sm:text-[13px] font-mono font-black bg-purple-500/20">
                  {summary.totalGsv.toFixed(3)}
                </td>
                <td className="py-2.5 px-0.5 text-right text-emerald-300 text-xs sm:text-[13px] font-mono font-black bg-emerald-500/25">
      ...
      {/* Modal for Certificate Pipeline Capacity Table */}
      {showPipelineModal && (
        <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-sm flex items-center justify-center p-4 overflow-y-auto animate-in fade-in duration-200">
          <div className="bg-slate-900 border border-slate-700 rounded-2xl max-w-3xl w-full max-h-[90vh] overflow-y-auto p-6 shadow-2xl relative">
            ...
            <PipelineCapacityTable
              vesselMeta={vesselMeta}
              onApplyPipelineVolume={(vol, label) => {
                setGlobalInput(prev => ({ ...prev, pipelineVolume: vol }));
